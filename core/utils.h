...
namespace common {
    ...
    template <typename T>
    std::optional<View> broadcastable(const Tensor<T>& t1, const Tensor<T>& t2) {
        ...
        std::optional<View> new_view{ smaller_dims };
        for (auto&& [shape, stride] : std::views::zip(new_view.value().shape, new_view.value().strides)) {
            if (shape == 1) { stride = 0; }
        }
